...
test('能把中文正文中风险裸 URL 宏改为显式 link 宏', () => {
  const source = [
    ...
    '注意：{url-spring-framework-docs}/testing.html[相关章节] 需要先阅读。',
    '通过设置环境变量来{url-paketo-docs-java-buildpack}/#runtime-jvm-configuration[配置 JVM 运行时环境]。',
    '一个典型项目会应用 {url-gradle-docs-groovy-plugin}[`groovy`]、{url-gradle-docs-java-plugin}[`java`] 或 {url-kotlin-docs-kotlin-plugin}[`org.jetbrains.kotlin.jvm`] 插件。',
    '显式 link:https://example.com/[链接] 保持不变。',
    '显式 link:{url-spring-framework-docs}/testing.html[相关章节] 保持不变。',
    '列表项开头的 URL 宏保持不变：',
  ...
  assert.equal(normalized.content, [
    ...
    '注意：link:{url-spring-framework-docs}/testing.html[相关章节] 需要先阅读。',
    '通过设置环境变量来link:{url-paketo-docs-java-buildpack}/#runtime-jvm-configuration[配置 JVM 运行时环境]。',
    '一个典型项目会应用 {url-gradle-docs-groovy-plugin}[`groovy`]、link:{url-gradle-docs-java-plugin}[`java`] 或 {url-kotlin-docs-kotlin-plugin}[`org.jetbrains.kotlin.jvm`] 插件。',
    '显式 link:https://example.com/[链接] 保持不变。',
    '显式 link:{url-spring-framework-docs}/testing.html[相关章节] 保持不变。',
    '列表项开头的 URL 宏保持不变：',
    '* https://docs.spring.io/spring-boot/4.1.0/api/java/[Spring Boot,role=link-external, window=_blank]',
