...
`prometheus` endpoint 以 Prometheus 服务器抓取所需的格式提供 Spring Boot 应用程序的指标。
...
当使用适当的 `Accept` 头调用时，endpoint 也可以生成 `application/openmetrics-text;version=1.0.0`，如下面的 curl 示例所示：
...
endpoint 使用查询参数来限制其返回的样本。下表显示了支持的查询参数：
