...
`metrics` endpoint 提供对应用程序指标的访问，用于诊断应用程序已记录的指标。
此 endpoint 不应在生产环境中被“抓取”或用作指标后端。
...
在这种情况下，`metrics` endpoint 仍然可能有用。
...
endpoint 使用查询参数通过其标签 xref:rest/actuator/metrics.adoc#metrics.drilling-down[深入] 指标。下表显示了支持的单个查询参数：
